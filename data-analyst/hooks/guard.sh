...
block() {
  echo "BLOCKED by data policy: $1" >&2
...
# --- Protecting the source data --------------------------------------------------
#
# Analysis writes new files. It never edits the export it was given, because a silently
# mutated source makes every past finding unreproducible.

if grep -Eiq "${SEP}(sudo[[:space:]]+)?(mv|cp)[[:space:]]+[^|;&]*\.(csv|tsv|xlsx|json|parquet)[[:space:]]*\$" <<<"$payload"; then
  block "moving or overwriting a source data file. Write a new file instead and leave the export untouched."
fi
exit 0
